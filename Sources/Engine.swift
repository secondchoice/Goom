...
class Sector: Decodable, Identifiable {
    struct Part: Decodable {
        ...
        init(from decoder: Decoder) throws {
            ...
            if let h = try values.decodeIfPresent(Float.self, forKey: .altHeight) { heights.append(h) }
...
class Wall: Decodable {
    ...
    init(base: Segment, top: Part, middle: Part, bottom: Part, frontSector: Sector, backSector: Sector) {
        self.base = base
        self.top = top
        self.middle = middle
        self.bottom = bottom
    ...
    required init(from decoder: Decoder) throws {
        ...
        if let lf = builder.sectors[frontSectorId] {
            ...
            throw DecodingError.dataCorrupted(.init(codingPath: decoder.codingPath, debugDescription: "Left floor ID \(frontSectorId) does not exist."))
        }
        if backSectorId != nil {
            if let rf = builder.sectors[backSectorId!] {
                rightSector = rf
            } else {
                throw DecodingError.dataCorrupted(.init(codingPath: decoder.codingPath, debugDescription: "Back floor ID \(backSectorId!) does not exist."))
            }
        } else {
            rightSector = nil
        }
...
class Texture: Decodable, Identifiable {
    ...
    required init(from decoder: Decoder) throws {
        ...
        if
            let path = Bundle.main.path(forResource: "Assets/" + name, ofType: "png"),
            let src = CGDataProvider(filename: path),
            let img = CGImage(pngDataProviderSource: src, decode: nil, shouldInterpolate: false, intent: .defaultIntent),
            let data = img.dataProvider?.data {
            let buffer = UnsafeBufferPointer(start: CFDataGetBytePtr(data), count: CFDataGetLength(data))
            width = img.width
            height = img.height
            pixels = []
            for c in 0 ..< width {
                let column = stride(from: c, to: c + width * height, by: width).map { UInt8(buffer[$0]) }
            ...
            pixels = []
            for c in 0 ..< width {
                let column = (0 ..< height).map { r in ((r & 32) ^ (c & 32)) != 0 ? UInt8(0) : UInt8(1) }
                pixels.append(column)
            }
        }
        if
            let path = Bundle.main.path(forResource: "Assets/" + name + "_mask", ofType: "png"),
            let src = CGDataProvider(filename: path),
            let img = CGImage(pngDataProviderSource: src, decode: nil, shouldInterpolate: false, intent: .defaultIntent),
            let data = img.dataProvider?.data {
            let buffer = UnsafeBufferPointer(start: CFDataGetBytePtr(data), count: CFDataGetLength(data))
            precondition(width == img.height)
            precondition(height == img.width)
            mask = []
            for c in 0 ..< width {
                let column = stride(from: c, to: c + width * height, by: width).map { UInt8(buffer[$0]) }
                mask!.append(column)
...
class Map: Decodable {
    ...
    func placeThings(atTime time: Float) {
        if let root = bsp.root {
            ...
            for thing in things {
                place(thingFragment: ThingFragment(ofThing: thing, atAngle: -player.angle), in: root, atPhase: time)
            }
        }
    }
}
...
private extension CodingUserInfoKey {
    static let worldBuilder = CodingUserInfoKey(rawValue: "worldBuilder")!
}
...
private func getBuilder(from decoder: Decoder) throws -> World.Builder {
    guard let builder = decoder.userInfo[.worldBuilder] as? World.Builder else {
        throw DecodingError.dataCorrupted(.init(codingPath: decoder.codingPath, debugDescription: "Decoder incorrectly initialized."))
    }
    return builder
}
